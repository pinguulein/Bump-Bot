...
class Bumps(commands.Cog):
    ...
    async def bump(self, ctx):
        ...
        if not server.get():
            ...
            return await ctx.send(embed=Embeds(f"Du musst den Server erst aufsetzen`{prefix}setup` um dies zu machen!").error())

        wait_msg = await ctx.send(embed=discord.Embed(
            title="⏳ Bumping...",
            description="Der Server wird grade gebumpt!",
            ...
            try:
                ...
                try:
                    await self.bot.get_guild(entry['_id']).owner.send(embed=discord.Embed(
                        title="⚠️ Server entfernt ⚠️",
                        description="Dein Server wurde aus der Datenbank entfernt, weil er einen Error verursacht hat. Der Bot benötigt die Permission Webhooks und Create Instant Invites.",
        ...
        done_message = await ctx.send(embed=discord.Embed(
            title="⏫ Server Bumped",
            description=f"Dein Bump war der `{success+fail}` Bump!\n✅ Erfolgreiche Bumps: `{success}`!\n❎ Es gab `{fail}´ Failbumps!",
            color=discord.Color.green()
        )
        .set_footer(text=f"Gebumpt von • {self.config['bot_name']}"))

        if settings["show_motd"]:
            await asyncio.sleep(settings["show_motd_wait"])
            return await done_message.edit(embed=discord.Embed(
                title="🗞️ Nachricht des Tages 🗞️",
