...
class SetPrefix(commands.Cog):
    ...
    async def setprefix(self, ctx, *, prefix=None):
        ...
        if not prefix or prefix == "=":
            ...
            return await ctx.send(embed=embeds.Embeds(f"Der Prefix wurde zurückgesetzt! `({Data('config').yaml_read()['prefix']})`").success())
        server.setPrefix(prefix)
        return await ctx.send(embed=embeds.Embeds(f"Der Prefix ist nun: `{prefix}`!").success())
